...
class vimconnector(vimconn.vimconnector):
    ...
    def _reload_connection(self):
        ...
        if self.reload_client:
           ...
           #self.glance_endpoint = self.keystone.service_catalog.url_for(service_type='image', endpoint_type='publicURL')
            ...
            self.ne_endpoint = 'http://223.194.33.59:9696' #'http://210.114.59.20:9696'
           #endpoint fixed(2015.12.02) if changed infra return again
    ...
    def delete_user(self, user_id):
        ...
        try:
            ...
            error_value=-vimconn.HTTP_Bad_Request
            error_text= type(e).__name__ + ": "+  (str(e) if len(e.args)==0 else str(e.args[0]))
        #TODO insert exception vimconn.HTTP_UnauthorizedR
        #if reaching here is because an exception
        if self.debug:
            print "delete_tenant " + error_text
        return error_value, error_text
        
    def new_project(self,tenant_name,tenant_description):
        ...
        try:
            ...
            self.keystone.roles.add_user_role('89a7c42c55204ca6901a243de786d50b','b72bd1477d794faf9942cb9bac85e5a3 ',tenant.id)
    ...
    def delete_project(self,project_id,project_tid,mgnt_network_id,router_id,EMA_iid,EMA_Address,floatingip_id):
        ...
        try:
          #  print tenant_id + " " + mgnt_network_id + " " +  ctrl_network_id + " " + " " + EMA_iid
            ...
            for s in servers_list:
                print("This server %s exists" % s.name)
                server_exists = True
		self.nova.servers.delete(s)
	
		
            
            floatingips_list=self.nova.floating_ips.list()
            for f in floatingips_list:
                
                self.nova.floating_ips.delete(f.id)
            ...
            self.neutron.remove_interface_router(router_id, { 'subnet_id' : subnetm['id'] } )
        
	    port_list = self.neutron.list_ports(router_id)
            #print ports
	    if port_list['ports']:
	       for i in range(len(port_list['ports'])):
	         if port_list['ports'][i]['tenant_id']==project_tid:
	              self.neutron.delete_port(port_list['ports'][i]['id'])
	    time.sleep(2)
	    self.neutron.remove_gateway_router(router_id)
            self.neutron.delete_router(router_id)
            
            
            neutron_list = self.neutron.list_networks()
            if neutron_list['networks']:
                for i in range(len(neutron_list['networks'])):
                    if neutron_list['networks'][i]['tenant_id']== project_tid:
                               
                        self.neutron.delete_network(neutron_list['networks'][i]['id'])
		         
            self.keystone.tenants.delete(project_tid)
        ...
        #if reaching here is because an exception
        if self.debug:
            print "delete_project " + error_text
        return error_value, error_text
    ...
    def new_project_network(self, project_id):
        self._reload_connection_tenant(project_id)
        try:
            body_mgmt = {'network': {'name': 'Management_Network',
                   'admin_state_up': True}}
            #body_ctrl = {'network': {'name': 'Control_Network',
             #      'admin_state_up': True}}
            
            netm = self.neutron.create_network(body=body_mgmt)
            #netc = self.neutron.create_network(body=body_ctrl)

            netm_dict = netm['network']
            #netc_dict = netc['network']
            mgnt_network_id = netm_dict['id']
            #ctrl_network_id = netc_dict['id']
            print('Management_Network %s created' % mgnt_network_id)
          #  print('Control_Network %s created' % ctrl_network_id)
            body_create_msubnet = {'subnets': [{'cidr': '10.100.0.0/16',
                         'ip_version': 4, 'network_id': mgnt_network_id}]}
           # body_create_csubnet = {'subnets': [{'cidr': '20.100.0.0/16',
           #              'ip_version': 4, 'network_id': ctrl_network_id}]}

            subnetm = self.neutron.create_subnet(body=body_create_msubnet)
            #subnetc = self.neutron.create_subnet(body=body_create_csubnet)
            
            print('Created Management_Network_subnet %s' % subnetm)
            #print('Created Contorl_Network_subnet %s' % subnetc)
           
            subnetm = self.neutron.list_subnets(network_id = mgnt_network_id)['subnets'][0]
            router = self.neutron.create_router( { 'router' : { 'name' : 'management-ext' } } ) 
            #time.sleep(1)
            #router = self.neutron.list_routers(name = 'management-ext')['routers'][0]
            self.neutron.add_interface_router(router['router']['id'], { 'subnet_id' : subnetm['id'] } )
            self.neutron.add_gateway_router(router['router']['id'], { 'network_id' : 'a504ec57-c9aa-40f3-9a07-f63e53a50be7'})
            
            return 1, mgnt_network_id, router['router']['id']
                
        finally:
            print("Execution completed")
            
    def vim_new_link(self, project_id, src_id, dst_id,cidr,src_ip, dst_ip, src_name, dst_name):
        self._reload_connection_tenant(project_id)
        try:
            body_mgmt = {'network': {'name': src_name + ',' + dst_name + 'link',
                   'admin_state_up': True}}
            #body_ctrl = {'network': {'name': 'Control_Network',
             #      'admin_state_up': True}}
            ...
            netm_dict = netm['network']
            #netc_dict = netc['network']
            link_iid = netm_dict['id']
            #ctrl_network_id = netc_dict['id']
            print(src_name + ', ' + dst_name + 'link' + 'created')
          #  print('Control_Network %s created' % ctrl_network_id)
            body_create_msubnet = {'subnets': [{'cidr': cidr,
                         'ip_version': 4, 'network_id': link_iid}]}
           # body_create_csubnet = {'subnets': [{'cidr': '20.100.0.0/16',
           #              'ip_version': 4, 'network_id': ctrl_network_id}]}

            subnetm = self.neutron.create_subnet(body=body_create_msubnet)
            #subnetc = self.neutron.create_subnet(body=body_create_csubnet)
            #print subnetm['subnets']['id']
            print('Created subnet %s' % subnetm)
            #print('Created Contorl_Network_subnet %s' % subnetc)
            body_value = {"port":{"admin_state_up":True,"name":"port","network_id":link_iid,"fixed_ips":[{"ip_address":src_ip}]}}
	    response = self.neutron.create_port(body=body_value)
            time.sleep(2)
	    print response
	    sinstance = self.nova.servers.find(id=src_id)
            print sinstance.interface_attach(response['port']['id'], None,None)
            time.sleep(3)
                	
            src_interface_portid =  response['port']['id']
            src_interface_macadd =  self.nova.servers.interface_list(src_id)[0].mac_addr
            
	    print src_interface_portid
            body_value = { "port":{"admin_state_up":True,"name":"port","network_id":link_iid,"fixed_ips":[{"ip_address":dst_ip}]}}
            response = self.neutron.create_port(body=body_value)
  	    print response
	    dinstance = self.nova.servers.find(id=dst_id)
	    dst_interface = dinstance.interface_attach(response['port']['id'], None,None)
            time.sleep(3)
	    dst_interface_portid =  response['port']['id']
            dst_interface_macadd =  self.nova.servers.interface_list(dst_id)[0].mac_addr
            print dst_interface_portid
            return 200, src_interface_portid, src_interface_macadd, dst_interface_portid,dst_interface_macadd, link_iid
    ...
    def vim_delete_link(self, project_id, sport_id, dport_id, src_id, dst_id, link_id):
        self._reload_connection_tenant(project_id)
        sinstance = self.nova.servers.find(id=src_id)
        sinstance.interface_detach(sport_id)
        self.neutron.delete_port(sport_id)
	time.sleep(1)
        dinstance = self.nova.servers.find(id=dst_id)
        dinstance.interface_detach(dport_id)    
        
	self.neutron.delete_port(dport_id)
        time.sleep(2)
        self.neutron.delete_network(link_id)
        
        return 200, link_id        
    
    def add_security_rules(self, project_id):
        self._reload_connection_tenant(project_id)
        sg_id = self.nova.security_groups.list()
        print sg_id 
        print "=========================="
        ...
        rule = self.neutron.create_security_group_rule(body)
        print rule
        print "======================================"
    ...
    def new_project_ema(self, project_id, network_id):
        self._reload_connection_tenant(project_id)
        image = self.nova.images.find(name="ema")
        nflavor = self.nova.flavors.find(name="m1.tiny")
        server = self.nova.servers.create(name="EMAgent", image = image.id, flavor = nflavor.id, nics = [{'net-id':network_id}], key_name = "")
        time.sleep(10)
        floating_ip = self.nova.floating_ips.create('ext-network')
        #instance = self.nova.servers.find(name="EMAgent")
        server.add_floating_ip(floating_ip)
        EMA_iid = server.id
        return 1, EMA_iid, floating_ip.ip, floating_ip.id
    
    def new_floatingip(self, project_id,vapp_iid):
        self._reload_connection_tenant(project_id)
        floating_ip = self.nova.floating_ips.create('ext-network')
        instance = self.nova.servers.find(id=vapp_iid)
        instance.add_floating_ip(floating_ip)
        return 200, floating_ip.ip, floating_ip.id
    
    
    def del_floatingip(self, project_id,vapp_iid,floatingip_id):
        self._reload_connection_tenant(project_id)
        instance = self.nova.servers.find(id=vapp_iid)
        #self.nova.floating_ips.delete('')
        
        self.nova.floating_ips.delete(floatingip_id)
        return 200, project_id
