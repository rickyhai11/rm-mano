...
def get_dbformat_compute_quotas_from_op():
    ...
    obj_quotas_compute = []
    json_data = get_quotas_limit_details()

    for tenant in tenantlist:
        print tenant.name
        json_quotas_compute = json_data[tenant.name]['quotas_compute']
        obj_quotas_compute.append(json.dumps(json_quotas_compute, sort_keys=False).encode('utf-8'))
    print "Print out obj_quotas_compute format-list of elements"
    print obj_quotas_compute
    return obj_quotas_compute



#---------------------------------------------------------------------------------------------------------------------------
# Get total capacity of compute resources from hyper-visor of compute node #TODO need to take into account multi-compute nodes scenario
#---------------------------------------------------------------------------------------------------------------------------
